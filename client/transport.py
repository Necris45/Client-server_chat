...
class ClientTransport(threading.Thread, QObject):
    ...
    def __init__(self, port, ip_address, database, username):
        ...
        try:
            ...
            logs.error('Timeout соединения при обновлении списков пользователей.')
    ...
    def connection_init(self, port, ip):
        ...
        logs.debug('Установлено соединение с сервером')
        ...
        logs.info('Соединение с сервером успешно установлено.')
    ...
    def create_presence(self):
        ...
        logs.debug(f'Сформировано {PRESENCE} сообщение для пользователя {self.username}')
        return out
    ...
    def process_server_ans(self, message):
        ...
        if RESPONSE in message:
            if message[RESPONSE] == 200:
                ...
                logs.debug(f'Принят неизвестный код подтверждения {message[RESPONSE]}')

        # Если это сообщение от пользователя добавляем в базу, даём сигнал о новом сообщении
        elif ACTION in message and message[ACTION] == MESSAGE and SENDER in message and DESTINATION in message \
                and MESSAGE_TEXT in message and message[DESTINATION] == self.username:
            logs.debug(f'Получено сообщение от пользователя {message[SENDER]}:{message[MESSAGE_TEXT]}')
